...
for (link in links) {
  i <- i + 1
  message("Procesando partido ", i, " / ", length(links))
  ...
  res <- tryCatch(
    {
      ...
      teamA_players <- pbp_things$playersTeamA %>%
        ...
        mutate(
          ...
          firstName = str_squish(as.character(firstName)),
          lastName = str_squish(as.character(lastName)),
      ...
      bind_rows(teamA_box, teamB_box) %>%
        mutate(
          competicion = pbp_things$game$competition$officialName,
          week = lubridate::isoweek(fecha),
          date = fecha,
